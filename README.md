...
- **多模型路由 + vendor-agnostic 配置**(multi-model routing with vendor-agnostic env namespace)—— `OpenAICompatibleProvider` 兼容任何 OpenAI-compatible 端点,模型 / base_url / api_key 通过 `STOCK_ASSISTANT_*` 统一命名空间配置,显式 pin `enable_thinking` 避免 vendor 默认行为漂移
- **健壮性 + 诚实降级**(robustness with honest degradation)—— Tool 失败时识别错误类型不 hallucinate;检索数据不全时坦白 "现有信息显示 X" 而非编造,保障 grounding 可信度
...
- **Python 3.12+**,**uv** 管理依赖
- **主 LLM**:xiaomi-v2.5(默认,经 dashscope OpenAI-compatible 接口)
...
```env
STOCK_ASSISTANT_API_KEY=你的-LLM-key

# 可选:覆盖默认 model / endpoint
...
│   ├── knowledge/
│   │   ├── store.py               # Knowledge: ingest_chunks + retrieve (ChromaDB)
│   │   └── embedder.py            # Embedding: dashscope text-embedding-v4
│   └── agents/
...
- **单 agent**:多 agent 编排有意推迟到 Phase 0.2。
